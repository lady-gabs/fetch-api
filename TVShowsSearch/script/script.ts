async function getTvShow() {
    let tvShow: string = (document.getElementById('tvShowInput') as HTMLInputElement).value;
    try {
        if (tvShow === '') {
            throw new Error('Insira um título.');
        }

        let response: Response = await fetch(`https://api.tvmaze.com/search/shows?q=${tvShow}`);
    ...
    if (data !== undefined) {
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].show.image === null) {
                result.innerHTML += `
                <li>
                    <div width="200px" id="nonImage-${i}" onclick="showModal(${i})">
                ...
                result.innerHTML += `
                <li>
                    <img id="image-${i}" width="200px" src="${data[i].show.image.medium}" alt="${data[i].show.name}" onclick="showModal(${i})">
                    <p><b>Nome:</b> ${data[i].show.name}</p>
                </li>
                `;
            }
...
async function showModal(index: number) {
    let modalInfos: any = document.querySelector('.tvShowInfos');
    ...
    if(modal.style.display == 'block') {
        ...
        switchModal();
        modalInfos.innerHTML = `
        <h1>${data[index].show.name}</h1>
        `;
        if (data[index].show.summary !== null) {
            modalInfos.innerHTML += `
        ...
        if(data[index].show.rating.average === null) {
            ...
            let rateDecimal: number = data[index].show.rating.average - Math.floor(data[index].show.rating.average);
            let arrayStars: string[] = [];
            for (let i = 0; i < Math.floor(data[index].show.rating.average); i++) {
                arrayStars.push(`
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="20px" width="20px" version="1.1" id="Capa_1" viewBox="0 0 47.94 47.94" xml:space="preserve">
            ...
            modalInfos.innerHTML += `
            <p>Rating: ${data[index].show.rating.average}</p>
            <div>
                ${arrayStars.join(' ')}
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" height="20px" width="20px" style="clip-path: inset(0 ${100-(rateDecimal*100)}% 0 0);" version="1.1" id="Capa_1" viewBox="0 0 47.94 47.94" xml:space="preserve">
                    <path style="fill:#ED8A19;" d="M26.285,2.486l5.407,10.956c0.376,0.762,1.103,1.29,1.944,1.412l12.091,1.757  c2.118,0.308,2.963,2.91,1.431,4.403l-8.749,8.528c-0.608,0.593-0.886,1.448-0.742,2.285l2.065,12.042  c0.362,2.109-1.852,3.717-3.746,2.722l-10.814-5.685c-0.752-0.395-1.651-0.395-2.403,0l-10.814,5.685  c-1.894,0.996-4.108-0.613-3.746-2.722l2.065-12.042c0.144-0.837-0.134-1.692-0.742-2.285l-8.749-8.528  c-1.532-1.494-0.687-4.096,1.431-4.403l12.091-1.757c0.841-0.122,1.568-0.65,1.944-1.412l5.407-10.956  C22.602,0.567,25.338,0.567,26.285,2.486z"/>
                </svg>
            </div>
            `;
